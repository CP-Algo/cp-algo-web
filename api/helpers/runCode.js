...
async function runCode(code, language, input, expected_output) {
  ...
  if (expected_output !== undefined) {
    expected_output = Buffer.from(String(expected_output)).toString('base64')
  }
  ...
  const pollForStatus = async (token) => {
    const { data } = await axios.get(`${judge0}/submissions/${token}/?base64_encoded=true&fields=status,stdout,stderr,time,memory,expected_output`)
